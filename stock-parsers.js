...
async function getAllStockData(forceUpdate = false) {
  try {
    // Skip timing checks if force update is requested
    if (!forceUpdate) {
      // Check if we should run now
      if (!isScheduledTime()) {
        console.log('[TIMING] Check: Not scheduled time');
        return null;
      }
      ...
      if (now - lastSuccessfulRun < 60000) { // 60 seconds
        console.log('[TIMING] Check: Too soon since last update');
        return null;
      }
    }
